...
A user space variable, such as \textit{particles}, may
...
the need.
  ...
  We illustrate our approach by looking at our example, shown in Figure~\ref{fig:ssio-bucket}.
  Current storage systems will be given a set of bytes with the total size  =
  to the size of the Particles (P) array
  + size of Field 1 + size of Field 2 + size of the Mesh. The Mesh would be a group
  of variables in the code,  typically  described by a data model.  In our example the storage
  system has no idea about different variables, and whether or not one variable or even a
  piece of a variable is more important to the user than the  others.  In  I/O systems
  such as ADIOS, the I/O system understands the data relationship between all of the 
  variables in the Mesh, and the meaning of each variable. Users can then request any
  portion of the variable to read during analysis, and even has the ability to temporary place
  variables in a DataSpace~\cite{docan2012dataspaces} for in situ analysis. 
  
  In this proposal we take a fresh approach to describing not only the information that
  ADIOS knows about, but we also describe the utility of data for different chunks. This
  allows the data chunks to be managed at a much lower level, and allows the SSIO layer
  to 
%  
%   
...
bridge the semantics gap between applications, middleware, and storage
...
In our example, when the user writes Field 1, they will be able to use a new API which
they can pass in a {\it plugin} function which can re-factor their data, which re-organizes
the data  in the SSIO later and allows the placement of the different chunks, (e.g. F1, F2) 
 in different
storage tiers, and for different lifetimes. 
Here the user use additional semantics to express that 
F will be contained in two chunks. For simplicity,  let's assume in our example for this
 that the data will use the system {\it precision} based re-organization. 
  The 
data is then broken into  two chunks where F1 is of higher value than F2 (e.g. F1 contains 
the first 3 digits of each double precision value, and the exponent and sign, and F2 contains
...
(2,(1 hour, 4 days, 100 days, 300 days). This allows the initial data chunks to be placed in 
~Figure{fig:ssio-bucket} b, and then later in ~\ref{fig:ssio-bucket} c.  
The data description may also be be stored within a
...
relationship to other data. In our example the relationship of the Mesh chunks and the
field Chunks would also allow the system to understand that the user will only want to
obtain chunks F2, with M2. 

When a user is reading this data at a later time the user can express how much time
they will be willing to allow the read operations to take place. For example, the user might
want to visualize F, which means they need to read in F along with the mesh.  The user
will also specify which time slices to read (one for a simple visualization at a single 
instance of time), and tell the system they are willing to wait 10 seconds. The system 
will take this additional information and deliver the highest value chunks it can in order
to meet this time constraint.  We will also allow additional information from the user so 
they can express rules about the possible accuracy of the data to read to allow the system
...
We will also develop a querying mechanism that allows an application
...
be issued to the storage system. This new query mechanism will allow users to
