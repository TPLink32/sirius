...
can be managed independently or collectively, depending on
the need.

  We illustrate our approach through a simple example in Figure~\ref{fig:ssio-bucket},
  in which the user attempts to write \textit{Particles}, \textit{Field 1},
  \textit{Field 2}, and \textit{Mesh} arrays. In particular, the \textit{Mesh} can be a group
  ...
  systems and storage are oblivious to the user space variables and the relationship between
  them. Recent advances in I/O middleware systems such as ADIOS \cite{liu_helloadios}
  allow for understanding and capturing the data semantics and relationship between all 
  ...
  In this project we will leverage the ability of plugging rich metadata from ADIOS, and further
  describe the utility of data for data chunks. This enables user data to be possibly managed at
  the storage system level, and allows the SSIO layer
  to 
%  
%   
...
Furthermore, when the user writes Field 1, they will be able to use a new API through which
they can pass in a {\it plugin} function that re-factors their data (\S~\ref{sec:data-refactor}),
which re-organizes data and facilitates the placement of the different chunks, e.g., F1 and F2, 
onto different storage tiers, with a different lifetime specified for each chunk. 
Here the user will uses additional semantics to express that 
F will consist of two chunks. For simplicity, let's assume in this example
 that system {\it precision} based re-organization will be used. 
  The 
