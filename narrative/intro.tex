...
new research into managing and storing the large volumes of data that are 
...
to facilitate efficient mapping of data objects
from the user space onto multiple storage tiers and enable
application-guided data reductions and transformations to address capacity and
...
{\em Address emerging storage architecture/technology landscape:}
...
data lifetime they can support, our approach will enable researchers to 
place more knowledge into the SSIO system so that data can be recast from 
a sequence of bytes to information, which the system can then place and 
optimize the time to gain knowledge from exascale simulations. 
...
is not storing data, but rather executing knowledge creation and extraction processes 
on the data. 
...
Our proposal will be responsive to Theme 2: {\em Scalable Storage Software Infrastructure} of the call, as summarize in Table~\ref{ab:call}.
\begin{table}[htbp]
\vspace{2ex}
\begin{center}
