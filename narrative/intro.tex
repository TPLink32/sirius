...
From the perspective of reading, this project will look at key I/O patterns such as checkpoint-restart (CR), in-situ analytics and post-processing. 
For example, in case of CR, we will support both, traditional file-based approaches leveraging fast storage tiers
such as NVRAM (non-volatile RAM), as well as memory-based approach based on retrieving
data directly from NVRAM. In case of in-situ and post-processing,  an important aspect is the latency -- here reducing latency and providing QoS 
 becomes important. 
 When someone is doing interactive analysis and expects that the data they read in is about
