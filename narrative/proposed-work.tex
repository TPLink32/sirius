...
\underline{Principle 1:} A knowledge-centric system , let user knowledge  to
...
We propose to re-design the layer in a highly integrated fashion where user
will place their intentions into the system and actions will dynamically and
statically take place to optimize for both the system and for individual
requests.

\underline{Principle 2}: Predictable Performance in the SSIO layer needs to be
...
users, which quite often mean that science is not performed well.

\underline{Principle 3:} In a data-centric SSIO system, we need user goals to
define the data policies. We introduce data-driven SSIO optimizations by
