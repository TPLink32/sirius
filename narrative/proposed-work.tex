...
\underline{Principle 1:} A knowledge-centric system , let user knowledge  to define data policies. Today Storage System and I/O
layers are written in a stove-pipe fashion, and quite often don't allow optimizations to take place. We propose to re-design the
layer in a highly integrated fashion where user will place their intentions into the system and actions will dynamically and statically
take place to optimize for both the system and for individual requests.

\underline{Principle 2}: Predictable Performance in the SSIO layer needs to be established so more science can be done on the
exascale Systems. Without predictable performance, the I/O layer performance must be guessed by the users, which quite often
mean that science is not performed well.
...
SSIO optimizations by enabling users to define data manipulations (re-organizations) at a high level and then we express
