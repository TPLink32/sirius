...
if test "x$with_datatap" = "xno"; then
	...
	if test x"$ac_with_infiniband" = xyes; then
		...
		CERCS_REQUIRE_PACKAGE(ptlpbio, thin_portal.h, libptlclient.a libptlserver.a)

		if test -n "$cercs_cv_ptlpbio_link_dir";then
			DT_LDFLAGS="$DT_LDFLAGS -L$cercs_cv_ptlpbio_link_dir"
			DT_LIBS="$DT_LIBS -lptlclient -lptlserver -lmxml -lbench -ldl"
			datatap=portals
		else
			temptest=disable
		fi
...
dnl directory given .. add it to search path with CERCS_REQUIRE_PACKAGE
	...
	if test x"$ac_with_infiniband" = xyes; then
		...
		if test -n "$cercs_cv_ptlpbio_link_dir";then
			#DT_LDFLAGS="$DT_LDFLAGS -L$cercs_cv_ptlpbio_link_dir"
			DT_LDFLAGS="$DT_LDFLAGS -L$withval/lib"
			DT_LIBS="$DT_LIBS -lptlclient  -lptlserver -lmxml -lbench -ldl"
                        DT_CPPFLAGS="-I$withval/include"
			datatap=portals
		else
			temptest=disable
		fi
